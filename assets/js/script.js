	...
	constructor() {
		...
		this.addEventListener("keydown", (event) => {
			...
			const tabs = Array.from(
				this.querySelectorAll('[role="tablist"] [role="tab"]')
			);
...
(function () {
	...
	// Toggle mobile nav
	var mobileNavBtns = [].slice.call(
		document.querySelectorAll(".js-details-target")
	);
...
var outputDelay = 300;
// ms before command output is shown
var cycleDelay = 5000;
// ms between going to next command
...
function showCommand(index, cycle = true) {
	...
	Array.from(
		document.querySelectorAll(".cli-window div[role=tabpanel]")
	).forEach(function (el) {
		el.removeAttribute("hidden");
	});

	// show current command while respecting animated ones
	Array.from(document.querySelectorAll(".command-" + index)).forEach(function (
		el
	) {
